...
if [ -n "$@" ]; then
	...
	if grep -q '+\|-[[:digit:]]\|\*\|\/\|\^' <<<"$input"; then
		if [ -n "$(command -v bc)" ]; then # has bc package on system
			...
			echo "$input = ""$( python3 -c "print( float(1)*$input )" )"
			...
			echo 'Please install bc, python3 or bcalc.'
		fi
	fi
    exit 0
fi
